...
from datasets.downstream.CHB_MIT_Scalp_EEG.preprocess_chb_mit_scalp_eeg import shift_and_save_patient
...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def on_test_epoch_end(self) -> None:
        ...
        metrics_list = ["accuracy", "precision", "recall",
                        "recall", "f1", "cohen_kappa", "roc_auc"]
        results = get_metrics(y_score.cpu().numpy(), label.cpu().numpy(), metrics_list, threshold=True, is_binary=True)

        # --------- 逐项 log ---------------
        for k, v in results.items():
            self.log(f"test_{k}", v, on_epoch=True, prog_bar=False, sync_dist=True)
    ...
    '''def _step(self, batch, stage: str):
        ...
        x_proc, logit = self.forward(x)
        loss = self.loss_fn(logit, label)

        # 1) collect prob of class 1
        prob1 = F.softmax(logit, dim=-1)[:, 1]
        self.metric_buf["y"].append(label.detach().cpu())
        self.metric_buf["p"].append(prob1.detach().cpu())

        # 2) log loss + basic acc
        preds = torch.argmax(logit, dim=-1)
    ...
    def _epoch_end(self, stage: str):
        ys = torch.cat(self.metric_buf["y"]).numpy()
        ps = torch.cat(self.metric_buf["p"]).numpy()
        preds = (ps > 0.5).astype(int)

        # compute metrics
        acc = metrics.accuracy_score(ys, preds)
        auc = metrics.roc_auc_score(ys, ps) if len(np.unique(ys)) == 2 else 0.0
        precision = metrics.precision_score(ys, preds, zero_division=0)
        recall = metrics.recall_score(ys, preds, zero_division=0)
...
path = "../datasets/downstream/CHB_MIT_Scalp_EEG/processed_data/"
...
print(f'x_shifted_path:{x_shifted_path}')
