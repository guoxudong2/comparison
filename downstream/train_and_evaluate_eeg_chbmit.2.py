...
改为按簇轮流留出做验证（Leave-One-Cluster-Out CV）。如果病人只有1或2个簇，则直接跳过训练并退出。
"""
...
def log_split_stats(name, idxs, y_all):
    y_split = y_all[idxs]
    n0, n1 = (y_split == 0).sum(), (y_split == 1).sum()
    print(f"{name:<5}  总片段={len(idxs):5d}   0:{n0:5d}  1:{n1:5d}   正例比例={n1/len(idxs):.3f}")

def count_preictal_segments(t0_array, seizure_times, label):
    print(f"\n[{label}] 每场发作在 SOP–SPH（-30min∼-1min）预警窗口内的 segment 数量：")
    for i, t in enumerate(seizure_times):
        mask = (t0_array >= t - 1800 -60) & (t0_array <= t - 60)
        print(f"Seizure {i+1:2d}: {mask.sum():3d} / 174")
...
class LitEEGPT(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if batch_idx == 0 and self.current_epoch % 1 == 0:
            unique, cnt = torch.unique(torch.argmax(logits, -1), return_counts=True)
            print(f"Epoch{self.current_epoch}: pred stat", dict(zip(unique.tolist(), cnt.tolist())))

            unique_y, cnt_y = torch.unique(y, return_counts=True)
            print(f"Epoch {self.current_epoch}: label stat", dict(zip(unique_y.tolist(), cnt_y.tolist())))
    ...
    def on_test_epoch_end(self) -> None:
        ...
        all_t0 = np.concatenate(self.test_t0s, axis=0)

        order = np.argsort(all_t0)
        all_t0 = all_t0[order]
        all_preds = all_preds[order]
        all_logits = all_logits[order]
        ...
        unique_pred, cnt_pred = np.unique(all_preds, return_counts=True)
        print("Test set prediction distribution:", dict(zip(unique_pred, cnt_pred)))

        unique_label, cnt_label = np.unique(all_labels, return_counts=True)
        print("Test set true label distribution:", dict(zip(unique_label, cnt_label)))
        ...
        for i in range(len(all_preds)):
            ...
            print(f'i-N+1:{i-N+1}, i+1:{i+1}window.sum:{window.sum()}')
            if window.sum() >= K:
        ...
        TP_event = 0
        FN_event = 0
        print(f'seizure_times:{seizure_times}')
        for t_ictal in seizure_times:
            print(f't_ictal:{t_ictal}')
            print(f'alarm_times:{alarm_times}, t_ictal-SOP_seconds:{t_ictal-SOP_seconds}, SPH_seconds:{t_ictal-SPH_seconds}')
        ...
        ita = FP_event / inter_h if inter_h > 0 else 0.0
        SOP_h = SOP_seconds / 3600.0
        p_hit = 1.0 - np.exp(-ita * SOP_h + 1e-15)
...
class CHBMITDataModule(pl.LightningDataModule):
    def __init__(self, pid: str, cfg: DotMap, val_cluster_idx: int = 0):
        """
        pid: 病人 ID，例如 "chb04"
        cfg: 配置字典，需包含 data.interpolation_len, data.batch, data.shift_dir
        val_cluster_idx: 当前折要留作验证的簇索引
        """
        super().__init__()
        self.pid = pid
        self.cfg = cfg
        self.val_cluster_idx = val_cluster_idx

    def setup(self, stage=None):
        base = os.path.join(self.cfg.data.shift_dir, self.pid)
        X = np.load(base + "_X.npy")
        y = np.load(base + "_y.npy")
        t0 = np.load(base + "_t0.npy")
        cid = np.load(base + "_cluster.npy")
        seizure_times = np.load(base + "_seizure_times.npy")

        # 所有有效簇（>=0）
        valid_clusters = np.unique(cid[cid >= 0])
        if len(valid_clusters) < 3:
            print(f"{self.pid}: 有效簇数 < 3，跳过。");
            sys.exit(0)

        test_cluster = valid_clusters[-1]
        val_cluster = valid_clusters[self.val_cluster_idx %
                                     (len(valid_clusters) - 1)]

        # 分割策略：同一个 cluster_id 的 0、1 同进同出
        #train_mask = (cid != test_cluster) & (cid != val_cluster)
        '''train_mask = (cid != test_cluster) & (cid != val_cluster) & (cid >= 0)
        val_mask = (cid == val_cluster)
        test_mask = (cid == test_cluster)'''

        # 新代码：仅使用远背景的0类 + 每簇中的1类样本
        train_mask = ((cid == -1) & (y == 0)) | ((cid != test_cluster) & (cid != val_cluster) & (y == 1))
        val_mask = ((cid == -1) & (y == 0)) | ((cid == val_cluster) & (y == 1))
        test_mask = ((cid == -1) & (y == 0)) | ((cid == test_cluster) & (y == 1))

        print("#train background-0 =", np.sum((y == 0) & (cid == -1) & train_mask))  # 应为 0
        print("#val   background-0 =", np.sum((y == 0) & (cid == -1) & val_mask))  # 0

        train_idxs = np.where(train_mask)[0]
        val_idxs = np.where(val_mask)[0]
        test_idxs = np.where(test_mask)[0]

        def _balance_split(idxs: np.ndarray) -> np.ndarray:
            """在一个索引集合里，把 interictal(0) 随机/等距采样到与 preictal(1) 同数。
               - 如果负样本 < 正样本，则有放回采样负样本补足
               - 返回新的 idxs（已排序，以免 DataLoader 打乱时间先后）"""
            y_split = y[idxs]
            pos_idx = idxs[y_split == 1]
            neg_idx = idxs[y_split == 0]

            n_pos, n_neg = len(pos_idx), len(neg_idx)
            if n_neg > n_pos:
                # 等距采样负样本 n_pos 个（也可以用 np.random.choice）
                keep_neg = neg_idx[np.linspace(0, n_neg - 1, n_pos, dtype=int)]
                new_idxs = np.sort(np.concatenate([pos_idx, keep_neg]))
            elif n_neg < n_pos:
                # 负样本太少，随机“复制”一些负样本补齐
                extra_neg = np.random.choice(neg_idx, size=n_pos - n_neg, replace=True)
                new_idxs = np.sort(np.concatenate([pos_idx, neg_idx, extra_neg]))
            else:
                new_idxs = np.sort(idxs)  # 本来就 1:1

            return new_idxs

        train_idxs = _balance_split(train_idxs)
        val_idxs = _balance_split(val_idxs)
        test_idxs = _balance_split(test_idxs)

        # --------- 统计 ----------
        log_split_stats("train", train_idxs, y)
        log_split_stats("val", val_idxs, y)
        log_split_stats("test", test_idxs, y)

        count_preictal_segments(t0[train_idxs], seizure_times, "train")
        count_preictal_segments(t0[val_idxs], seizure_times, "val")
        count_preictal_segments(t0[test_idxs], seizure_times, "test")
        ...
        self.train_ds = Subset(full_ds, train_idxs.tolist())
        self.val_ds = Subset(full_ds, val_idxs.tolist())
        self.test_ds = Subset(full_ds, test_idxs.tolist())
...
if __name__ == "__main__":
    ...
    # 如果要在多个簇上轮流做 LOOCV，可以在外部循环设置 val_cluster_idx
    # 这里只默认第 0 个簇作为验证；如需第 i 簇，请在 cfg.train 里加： fold_idx: i
    fold_idx = int(getattr(cfg.train, "fold_idx", 0))

    dm = CHBMITDataModule(pid=pid, cfg=cfg, val_cluster_idx=fold_idx)
    dm.setup()
    ...
    tb_logger  = pl.loggers.TensorBoardLogger("./logs_eegpt/", name="EEGPT", version=f"{pid}_fold{fold_idx}")
    csv_logger = pl.loggers.CSVLogger("./logs_eegpt/", name="EEGPT_csv", version=f"{pid}_fold{fold_idx}")
    ...
    print(f"== PID={pid} , Fold={fold_idx},  #train={len(dm.train_ds)}, #val={len(dm.val_ds)}, #test={len(dm.test_ds)} ==")
