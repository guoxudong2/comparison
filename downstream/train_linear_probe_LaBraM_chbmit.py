...
class LitEEGPTCausal(pl.LightningModule):
    ...
    def __init__(self):
        ...
        self.head = LinearWithConstraint(4800, 2, max_norm=1) #我加的
    ...
    def on_test_epoch_end(self) -> None:
        ...
        pred_class = torch.argmax(y_score, dim=1)
...
for i in range(1,10):
    ...
    trainer = pl.Trainer(accelerator='cuda',
                         ...
                         callbacks=callbacks,
                         logger=[pl_loggers.TensorBoardLogger('./logs/', name="LaBraM_CHBMIT_tb", version="subject{}".format(i)),
                                 pl_loggers.CSVLogger('./logs/', name="LaBraM_CHBMIT_csv")])
...
trainer = pl.Trainer(accelerator='cuda',
                         max_epochs=max_epochs,
                         callbacks=callbacks,
                         logger=[pl_loggers.TensorBoardLogger('./logs_labram/', name="LaBraM_CHBMIT_tb", version="chb01"),
                                 pl_loggers.CSVLogger('./logs_labram/', name="LaBraM_CHBMIT_csv")])
...
parser.add_argument('--cfg', default='../conf/mylabram.yaml')
args = parser.parse_args()
cfg = load_cfg(args.cfg)

shift_dir = cfg['data']['shift_dir']
x_files = sorted(glob.glob(os.path.join(shift_dir, '*_X_shift.npy')))
assert x_files, f'No *_X_shift.npy found under {shift_dir}'
...
for x_path in x_files:
    ...
    check_label = np.load(y_path)
    print(f"[{pid}] Label distribution:", np.bincount(check_label))

    model = LitEEGPTCausal()
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.loss_fn = nn.CrossEntropyLoss(weight=torch.tensor(cls_w, dtype=torch.float32, device=device))
    max_epochs = 100
    steps_per_epoch = math.ceil(len(tr_ld))
    max_lr = 4e-4

    # —— 日志 / 回调
    tb_logger  = pl_loggers.TensorBoardLogger('./logs_labram/', 'tb', version=pid)
    csv_logger = pl_loggers.CSVLogger('./logs_labram/', 'csv', version=pid)
    lr_monitor = LearningRateMonitor(logging_interval='step')
    # 如果需要保存 ckpt，把 enable_checkpointing=True 并加 ModelCheckpoint
    trainer = pl.Trainer(
        max_epochs=max_epochs,
